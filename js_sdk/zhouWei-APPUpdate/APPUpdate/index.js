...
const getServerNo = function(version,isPrompt = false, callback) {
	...
	// 	}
	// });
	console.log(api.get_new_version)
...
const getDownload = function(data) {
    ...
    dtask = plus.downloader.createDownload(data, {
	...
	getCurrentNo(versionInfo => {
		getServerNo(versionInfo,isPrompt, res => {
			...
			if (res.appType == 1) {
				...
				downloadUrl = res.iosUrl
			}
			
			if (res.upDateFlag) {
				if (/\.wgt$/i.test(downloadUrl)) {
					getDownload(downloadUrl);
				} else if(/\.html$/i.test(downloadUrl)){
